        ...
        <form action="{{ url('penjualan/') }}" method="POST" class="form-horizontal">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="penjualan_kode" name="penjualan_kode"
                    value="{{ $penjualan->penjualan_kode }}">
            ...
            <div class="form-group">
                ...
                <input type="time" class="form-control" id="waktu" name="waktu"
                    value="{{ \Carbon\Carbon::parse($penjualan->penjualan_tanggal)->format('H:i') }}">
                @error('waktu')
                <small class="form-text text-danger">{{ $message }}</small>
                @enderror
            </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input barang-checkbox" type="checkbox"
                                id="barang_{{ $barang->barang_id }}" name="barang[{{ $barang->barang_id }}][id]"
                        ...
                        <div class="input-group input-group-sm mb-3">
                            <input type="number" class="form-control barang-jumlah"
                                id="barang_{{ $barang->barang_id }}_jumlah"
                                name="barang[{{ $barang->barang_id }}][jumlah]" min="1" step="1" value="1"
                                data-id="{{ $barang->barang_id }}">
                            <input type="hidden" class="form-control barang-harga"
                                id="barang_{{ $barang->barang_id }}_harga"
                                name="barang[{{ $barang->barang_id }}][harga]" value="{{ $barang->harga_jual }}">
                            <input type="text" class="form-control barang-subtotal"
                                id="barang_{{ $barang->barang_id }}_subtotal"
                                name="barang[{{ $barang->barang_id }}][subtotal]" readonly>
                            <div class="input-group-append">
                                <button class="btn btn-outline-secondary btn-tambah" type="button" data-action="tambah"
                                    data-id="{{ $barang->barang_id }}"><i class="fas fa-plus"></i></button>
                                <button class="btn btn-outline-secondary btn-kurang" type="button" data-action="kurang"
                                    data-id="{{ $barang->barang_id }}"><i class="fas fa-minus"></i></button>
                            </div>
                        </div>
                    </div>
                    @endforeach
